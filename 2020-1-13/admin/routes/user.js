...
route.get('/login', (req, res) => {
	...
	jwt.verify(authorization, secret,(err,data)=>{
		...
		res.json({
			code:0,
			token:jwt.sign({name:data.name,jobId:data.jobId},secret,{expiresIn: 60*60})
		});
		return;
	});
...
/*
	...
	allcustomer:可以获取所有客户
	departcustomer:可以获取部门客户
	resetpassword:修改密码
*/
route.get('/power', (req, res) => {
	...
	jwt.verify(authorization, secret,(err,data)=>{
		...
		console.log(powerList[data.jobId],data.jobId,data);
		const createObj = powerList[data.jobId].split('|');
