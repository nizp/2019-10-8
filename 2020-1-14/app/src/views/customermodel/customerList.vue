...
import {customerListAPI,resetPassAPI,userListAPI,userSelectAPI,userDeleteAPI,allListAPI,userInfoAPI,changeUserAPI} from '../../api/api';
    export default {
        beforeRouteUpdate(to,from,next) {
            // console.log(to)
            this.getList({lx:to.params.id === 'my'?'my':'all'});
            next();
        },
        created(){
            let session = sessionStorage.getItem('power');
            if(session){
                const {power,id} = JSON.parse(session)
                this.userhandler = power.includes('userhandle')
                this.resetPass = power.includes('resetpassword')
                this.userId = id;
            }
            console.log(this.$route.params.id)
           
             this.getList({lx:this.$route.params.id === 'my'?'my':'all'});
